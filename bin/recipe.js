...
const HOOK_CMD =
  'node "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/recipe/recipe-hook.js"';
...
async function main() {
  ...
  if (!cmd || !Object.hasOwn(commands, cmd)) {
    console.log(
      "usage: recipe <init | use <ref> | search <term> | publish | list | off | new | test <prompt> | lint [ref] | discover | export [--out file] | reload | setup [--yes]>",
    );
...
function init() {
  ...
  for (const e of settings.hooks.UserPromptSubmit) {
    e.hooks = (e.hooks || []).filter(
      (h) =>
        !(
          String(h.command || "").includes(
            ".claude/hooks/recipe/recipe-hook.js",
          ) && h.command !== HOOK_CMD
        ),
    );
  }
  const entries = (settings.hooks.UserPromptSubmit =
    settings.hooks.UserPromptSubmit.filter((e) => e.hooks.length > 0));
  const installed = entries.some((e) =>
    (e.hooks || []).some((h) => h.command === HOOK_CMD),
  );
  if (!installed)
    entries.push({ hooks: [{ type: "command", command: HOOK_CMD }] });
  fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2) + "\n");
  console.log(
    "recipe: hook installed. Next: recipe use <gh:user/repo | ./file.md>",
  );
}
...
async function use(ref) {
  if (!ref)
    throw new Error(
      "usage: recipe use <gh:user/repo[/path] | mkt:slug | ./file.md>",
    );
  ...
  if (missing.length) {
    console.log(
      `⚠ ${missing.length} required skill(s) not installed: ${missing.map(([n]) => n).join(", ")}`,
    );
...
function skillInstalled(name) {
  if (process.env.RECIPE_SKILLS_DIRS) {
    return process.env.RECIPE_SKILLS_DIRS.split(path.delimiter).some((dir) =>
      ...
      for (const plugin of fs.readdirSync(mpDir, { withFileTypes: true })) {
        ...
        for (const version of fs.readdirSync(pluginDir, {
          ...
          for (const sub of ["skills", path.join(".claude", "skills")]) {
            if (
              fs.existsSync(
                path.join(pluginDir, version.name, sub, name, "SKILL.md"),
              )
            )
              return true;
          }
...
async function reload() {
  ...
  if (!fs.existsSync(src))
    throw new Error("no source recorded — run: recipe use <ref>");
...
async function lint(ref) {
  ...
  if (ref) {
    ...
    const file =
      active && fs.existsSync(active)
        ? active
        : fs.existsSync("recipe.md")
          ? "recipe.md"
          : null;
    if (!file) {
      console.log(
        "recipe: nothing to lint (no ref, no active recipe, no ./recipe.md)",
      );
      return;
    }
  ...
  for (const i of recipe.ingredients) {
    if (i.tag !== "always" && !routes[i.tag]) {
      errors.push(
        `ingredient [${i.tag}] "${i.name}" has no route — it can never inject`,
      );
    }
  }
  for (const ch of Object.keys(skills)) {
    if (ch !== "always" && !routes[ch])
      errors.push(`skills channel "${ch}" has no route`);
  }
  for (const [name, cmd] of Object.entries(requires)) {
    if (typeof cmd !== "string" || !cmd.trim()) {
      errors.push(
        `requires "${name}" has an empty or non-string install command`,
      );
    }
  }
  ...
  for (const [ch, names] of Object.entries(skills)) {
    for (const n of names) {
      if (!Object.hasOwn(requires, n))
        warnings.push(`skill "${n}" (channel "${ch}") is not in requires:`);
    }
  }
  ...
  if (alwaysWords > 500) {
    warnings.push(
      `[always] bodies total ${alwaysWords} words — they ride every prompt, keep them lean`,
    );
  }
  const seen = new Set();
  for (const i of recipe.ingredients) {
    if (seen.has(i.name))
      warnings.push(`duplicate ingredient name "${i.name}"`);
    seen.add(i.name);
  }
  ...
  for (const ch of Object.keys(recipe.meta.routes || {})) {
    if (!tags.has(ch) && !skills[ch])
      warnings.push(
        `route "${ch}" fires nothing — no [${ch}] ingredient or skills: entry`,
      );
  }
  ...
  if (!errors.length && !warnings.length)
    console.log(`recipe: ${recipe.meta.name} — clean`);
...
function discover() {
  ...
  const channels = [
    ...new Set(
      recipe.ingredients.map((i) => i.tag).filter((t) => t !== "always"),
    ),
  ];
  ...
  for (const ch of channels) {
    const pct = prompts.length
      ? Math.round((fired[ch] / prompts.length) * 100)
      : 0;
  ...
  for (const ch of Object.keys(counts)) {
    const words = Object.entries(counts[ch])
      ...
      .map(([w]) => w);
    if (words.length)
      console.log(`suggest: routes.${ch} += ${words.join(", ")}`);
  }
}
...
const MKT =
  process.env.RECIPE_MKT_BASE || "https://recipe-kit-marketplace.vercel.app";
...
function fetchGh(ref) {
  ...
  const base =
    process.env.RECIPE_GH_BASE || "https://raw.githubusercontent.com";
...
async function search(term) {
  ...
  if (!Array.isArray(rows))
    throw new Error("marketplace search: unexpected response");
  if (!rows.length) {
  ...
  for (const r of rows) {
    console.log(
      `  ${r.slug}  ${r.name} — ${r.description}  (♥${r.likes} · ${r.author})`,
    );
  }
...
function list() {
  ...
  const active = fs.existsSync(ptr)
    ? fs.readFileSync(ptr, "utf8").trim()
    : null;
...
function scaffold() {
  ...
  console.log(
    "recipe: recipe.md created — edit it, then: recipe use ./recipe.md",
  );
}
