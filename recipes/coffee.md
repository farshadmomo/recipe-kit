...
MUST-USE (correctness — invoking these never constrains design, skipping them ships bugs): /threejs-loaders whenever loading any 3D model, texture, or HDR; /gsap-react for GSAP setup and cleanup in React; a /gsap-performance pass before shipping any motion; /ecommerce-kit for any cart, checkout, or money logic; /seo for schema and meta work. MUST-USE is a checklist, not an autopilot: invoke a skill when its domain genuinely comes up, and scale the effort to the task — a two-section static page needs correct schema and meta, not the full audit suite; a scene with no external assets gives /threejs-loaders nothing to do. Satisfy the intent, skip the ceremony.
CONSULT (craft reference — creativity stays yours): /ui-ux-pro-max, /frontend-design, /web-design, /impeccable, /gsap-scrolltrigger, /animejs. Read them for technique and API truth, then close them and design from your own taste. Never copy their example layouts, palettes, or component patterns — the design must come from this recipe's vibe pushed through your own judgment.
...
## [stack] tech
Next.js (App Router) + Tailwind CSS. React Three Fiber + drei for 3D. GSAP + Lenis + Anime.js v4 for motion. next/font for typography, next/image for media. Static-first (SSG); server actions only if a form/order flow demands it. Deploy on Vercel. No UI kits — every component custom.

## [seo] search
MUST use /seo for schema and meta work — scale it to the site: a landing page needs correct schema, meta, and alt text; save the full audit tooling for multi-page builds. JSON-LD CafeOrCoffeeShop schema (hours, geo, menu), per-page meta + OG images matching the art direction, semantic headings, image alt text with brand voice. Target green Core Web Vitals despite heavy motion — motion must never block LCP.

## [backend] server
Keep it static wherever possible: menu/products as typed local data (MDX or JSON). Contact/order form via a Next.js server action. No database until a real ordering feature demands one.
