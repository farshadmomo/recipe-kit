...
## [animation] motion
Choreography is yours — invent the moves. Lenis + GSAP ScrollTrigger. If GSAP earns its place over platform primitives: MUST /gsap-react for setup/cleanup + a /gsap-performance pass before shipping — 60fps or cut the effect. CONSULT /gsap-scrolltrigger and /gsap-timeline for technique, /animejs for micro-interactions. Idea starters, not a list: lineup names that slam in with stagger, a stage map that lights up as you scroll past each act, marquee tickers that reverse on scroll direction, ticket button with a squash worth pressing twice. gsap.matchMedia + prefers-reduced-motion always.
